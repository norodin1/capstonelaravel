<x-layout>
    ...
    <div class="container mx-auto p-6 lg:flex lg:space-x-6">
        ...
        <div class="w-full lg:w-2/3 bg-white p-6 rounded-lg shadow-md">
            ...
            <form action="{{ route('listing.cart.completeCart') }}" method="POST" class="space-y-4">
                @csrf
                <!-- Reference number -->
                <div>
                    <label class="block text-sm font-medium text-gray-700">Enter reference number</label>
                    <input type="text" name="reference" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-gray-500 focus:ring-gray-500" placeholder="" required>
                </div>
        ...
        <div class="w-full lg:w-1/3 bg-white p-6 rounded-lg shadow-md mt-6 lg:mt-0">
            ...
            <div class="space-y-4">
                ...
                <div class="flex justify-between">
                    <span>{{ $cart->itemDetails->itemName }} ({{ $cart->qty }})</span>
                    <span>$ {{ $cart->qty * $cart->itemDetails->itemPrice }}</span>
                </div>
